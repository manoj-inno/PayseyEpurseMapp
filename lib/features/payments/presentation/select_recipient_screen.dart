                  ...
                  child: BlocBuilder<GetRecipientsBloc, GetRecipientsState>(
                    builder: (context, state) {
                      ...
                      if (state is ContactsLoading) {
                        ...
                        debugPrint("✅ Total contacts: ${state.contacts.length}");
                        debugPrint("✅ Filtered contacts: ${state.filteredContacts.length}");
                        debugPrint("✅ Beneficiaries: ${state.beneficiaries.length}");
                        debugPrint("✅ Contacts: ${state.contacts.map((c) => '${c.name}: ${c.number}').toList()}");
                        debugPrint("✅ Beneficiaries: ${state.beneficiaries.map((b) => '${b.label}: ${b.mobile}').toList()}");
                        return ListView.builder(
                          physics: const BouncingScrollPhysics(),
                          ...
                          itemBuilder: (context, index) {
                            ...
                            debugPrint("📱 List item $index: ${contact.name} (${contact.number}) - Beneficiary: $isBeneficiary");
                            ...
                            return ListTile(
                              ...
                              trailing: isBeneficiary
                                  ...
                                  : TextButton(
                                      onPressed: () {
                                        debugPrint("📤 Inviting contact: ${contact.name} (${contact.number})");
                              ...
                              onTap: isBeneficiary
                                  ? () {
                                      debugPrint("💸 Navigating to payments for beneficiary: ${contact.name} (${contact.number})");
                                      
                                      // Find the matching beneficiary to get user_id
                                      final matchingBeneficiary = state.beneficiaries.firstWhere(
                                        (b) => _normalizeMobileNumber(b.mobile) == _normalizeMobileNumber(contact.number)
                                      );
                                      
                                      debugPrint("💸 Beneficiary user_id: ${matchingBeneficiary.userId}");
                                      ...
                                      context.navigateTo(
                                          ...
                                          arguments: PaymentsArguments(
                                              ...
                                              userId: matchingBeneficiary.userId.toString()));
                                    }
                                  : null,
                            );
                          },
